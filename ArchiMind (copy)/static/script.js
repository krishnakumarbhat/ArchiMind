document.addEventListener('DOMContentLoaded', () => {
    ...
    const setLoading = (isLoading, message = 'Analyzing repository...') => {
        ...
        if (isLoading) {
            const target = loadingIndicator.querySelector('div');
            if (target) {
    ...
    const pollStatus = (analysisId) => {
        const interval = setInterval(async () => {
            try {
                const statusUrl = analysisId ? `/api/status?analysis_id=${analysisId}` : '/api/status';
